...
async function run() {
    ...
    for (const page of existingPages) {
        if (deleteSlugs.includes(page.slug as string)) {
            console.log(`Deleting old page: ${page.slug}`)
    ...
    console.log('Creating complete legal pages...')
    ...
    console.log(`Created Privacy Policy id: ${privacyPolicy.id}`)
    ...
    console.log(`Created Terms & Conditions id: ${termsAndConditions.id}`)
    ...
    console.log(`Created Shipping & Return Policy id: ${shippingReturnPolicy.id}`)

    // 4. Update Header
    console.log('Fetching Global Header...')
    ...
    if (footer && footer.columns) {
        ...
        try {
            await payload.updateGlobal({
                slug: 'footer',
                data: {
                    columns: updatedFooterColumns as any
                }
            })
            console.log('Global Footer updated successfully.')
        } catch (e) {
            console.error('Failed Footer Update', e)
        }
    }
